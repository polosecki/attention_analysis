...
%% Make histograms of bRTs condidioned on different RT times
...
hs=subplot(2,2,1);
box off
[~, BRT_abs, ~, nBRT_abs] = get_distribution_from_total_behav(total_behav,'low',1,true);
[RT_abs, ~, ~, ~] = get_distribution_from_total_behav(total_behav,'none',1,false);
...
xlabel('Time from surface onset (s)')
ylabel('%')
title('M1 (RTs in second decile)')
yl=ylim;
%vtx=[min(RT_abs) min(RT_abs) prctile(RT_abs,10) prctile(RT_abs,10)];
vtx=[prctile(RT_abs,10) prctile(RT_abs,10) prctile(RT_abs,20) prctile(RT_abs,20)];
...
legend_vals={'cued surface t_{PME\_OT}','distractor surface t_{PME\_OT}','RT range'};
...
hs=subplot(2,2,2);
box off
[~, BRT_abs, ~, nBRT_abs] = get_distribution_from_total_behav(total_behav,'low',2,true);
[RT_abs, ~, ~, ~] = get_distribution_from_total_behav(total_behav,'none',2,false);
...
hs=subplot(2,2,3);
box off
[~, BRT_abs, ~, nBRT_abs] = get_distribution_from_total_behav(total_behav,'high',1,true);
[RT_abs, ~, ~, ~] = get_distribution_from_total_behav(total_behav,'none',1,false);
...
xlabel('Time from surface onset (s)')
ylabel('%')
title('M1 (RTs in last decile)')
yl=ylim;
vtx=[prctile(RT_abs,90) prctile(RT_abs,90) max(RT_abs) max(RT_abs)];
vty=[yl(1) yl(2) yl(2) yl(1)];
hp=patch(vtx,vty,[.5 .5 .5]);
set(hp,'facealpha',.5)
...
hs=subplot(2,2,4);
[~, BRT_abs, ~, nBRT_abs] = get_distribution_from_total_behav(total_behav,'high',2,true);
[RT_abs, ~, ~, ~] = get_distribution_from_total_behav(total_behav,'none',2,false);
...
if save_fig
    export_fig /Freiwald/ppolosecki/harbor/BRT_dist_figure.png -png -transparent
    pause(4)
    plot2svg('/Freiwald/ppolosecki/harbor/BRT_dist_figure.svg',f)
end