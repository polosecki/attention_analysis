function [psth] = calc_psth(tstamps,t_start,t_end,t_align,tres,Num,Den,fdelay,dec)
...
psthdata = zeros(length(t_start),maxbw);
bins = 1:maxbw;
%fprintf('maxbw = %d\n',maxbw);

% Process, trial by trial
for tk = 1:length(t_start)
    odata = zeros(1,maxbw);
    otstamps = tstamps(tstamps>=t_start(tk) & tstamps <= t_end(tk));
    % Refer all spikes to the alignment point (timestamps may get negative)
    oidx = round((otstamps-t_align(tk))/tres)+align_offset;
    odata = hist(oidx,bins);

    bws = round((t_start(tk)-t_align(tk))/tres) + align_offset;
    bwe = round((t_end(tk)-t_align(tk))/tres) + align_offset;
