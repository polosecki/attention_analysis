function [results]= measure_timing_of_effects_fun(monkey,area,use_high_res_data)
...
attention_long_enough=nan(size(all_cell_results,1),1);
attention_duration=nan(size(all_cell_results,1),1);
attention_onset=nan(size(all_cell_results,1),1);
saccade_onset=nan(size(all_cell_results,1),1);
saccade_long_enough=nan(size(all_cell_results,1),1);


for cell_no=1:size(all_cell_results,1)
...
if isempty(is_attn_long_enough);is_attn_long_enough=true;end
    attention_long_enough(cell_no)=is_attn_long_enough;
    attention_duration(cell_no)=attn_duration;
attention_onset(cell_no)=t_split_attn;
...
saccade_onset(cell_no)=t_split_sacc;
saccade_long_enough(cell_no)=is_sacc_long_enough;
